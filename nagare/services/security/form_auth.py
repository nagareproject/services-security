...
class Authentication(common.Authentication):
    ...
    def get_principal(self, request, response, **params):
        ...
        principal, credentials = self.get_principal_from_params(request.params)
        if (principal is None) or (credentials is None):
            # Second, search into the cookies
            principal, credentials = self.get_principal_from_cookie(request.cookies)

        if credentials is None:
            principal = None

        return principal, {'password': credentials}
    ...
    def handle_request(self, chain, request, response, session=None, **params):
        ...
        if user.is_expired:
            ...
            if (self.max_age is not None) or ((principal is not None) and (credential is not None)):
                response.set_cookie(
                    self.cookie_name,
                    branca.Branca(self.key).encode(json.dumps(user.get_id())),
